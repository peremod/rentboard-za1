...
- The frontend deploy workflow needs these GitHub Actions secrets set (Settings → Secrets and variables → Actions): `VERCEL_TOKEN`, `VERCEL_ORG_ID`, `VERCEL_PROJECT_ID`. It deploys `main`/`master` → production, `develop` → staging, matching `CONTRIBUTING.md §8`. **They are not set, so it has never successfully deployed.** The backend no longer has a deploy workflow — Railway was dropped and Render deploys `develop` from `render.yaml` without GitHub, so `RAILWAY_TOKEN` is no longer needed.
