...
| 12 | **The Lighthouse gate has never been green** — it has run twice, both red. Fixed in v1.58.0: two `"//1"`/`"//2"` comment keys sat inside `assertions`, where every key is an audit id, so LHCI failed on "is not a known audit" twice per URL; and `render-blocking-resources` was asserted with `maxNumericValue`, which reads NaN on an opportunity audit in Lighthouse 12. Twelve of the nineteen failures were the config reporting on itself. **What remains is real and pre-dates v1.58.0** — measured locally on the same production build the CI job uses: performance 0.76–0.83 against a 0.9 floor, LCP 3.5–4.2s against 2.5s, `uses-text-compression` flagging 382–386 KiB because the SSR server has never gzipped (Vercel compresses at its edge, so production is fine and the gate is measuring a condition that only exists in the test harness — one `compression()` line makes it representative); three `<select class="search-select">` on the home page hero with no accessible name (WCAG 4.1.2); `heading-order` on `/` and `/how-it-works`; `color-contrast` on `/how-it-works`. None caused by the WhatsApp pass — develop's own scheduled run failed with the same category scores | ⚠️ **Config fixed; four real findings open** | `lighthouserc.json`, `frontend/src/server.ts`, home hero search |
...
| Rent tracking — tenant side | ⬜ **Open.** The landlord can mark a month and the tenant is reminded over WhatsApp, but there is no tenant-facing rent screen: the dispute path exists only in the API, so the reminder's own instruction to "say so on Mastande" cannot be followed in the UI. Gap Y3 in `docs/FLOW-AUDIT.md`. Build before offering rent tracking to landlords |
