...
| 24 | **Production was a static deployment, and every non-prerendered URL 404'd.** Answered, by asking the live site rather than reading the config: `/`, `/pricing`, `/legal/terms` and `/advertise` returned 200; **`/auth/login`, `/tenant/dashboard` and every `/rooms/:id` returned 404** — a 79-byte `text/plain` 404 from Vercel itself, not the app. Every room link ever shared on WhatsApp was dead, and so was the login page. Two faults, both in `frontend/vercel.json`: `outputDirectory` named the browser bundle, so the `server.mjs` Angular builds on every deploy was published nowhere (which is what made the whole SSR layer inert, rows 18 and 20 included); and the SPA fallback rewrote unmatched paths to `/index.html`, which `"cleanUrls": true` makes unreachable — `/index.html` is a 308, so the fallback resolved to nothing. Fixed in v1.74.0 by removing both, with the Output Directory override cleared in the Vercel project so the Angular preset deploys the server as a function. `scripts/verify-build.sh` asserts both faults now; **nothing in this repo had ever read `vercel.json`**, which is why a config that served 404s to real visitors passed every gate for its whole life | ✅ **Fixed — verify against the deploy** | `frontend/vercel.json`, `docs/DEPLOYMENT.md §4`, `scripts/verify-build.sh` |
