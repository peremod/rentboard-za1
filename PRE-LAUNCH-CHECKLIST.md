...
| 24 | **Production is a static deployment, and deep links 404'd on it.** Measured against the live site: `/`, `/pricing`, `/legal/terms` and `/advertise` returned 200; **`/auth/login`, `/tenant/dashboard` and every `/rooms/:id` returned 404** — a 79-byte `text/plain` 404 from Vercel, not the app. Every room link ever shared on WhatsApp was dead. Cause: the SPA fallback rewrote to `/index.html`, which `"cleanUrls": true` makes unreachable (it is a 308), so the fallback resolved to nothing. Fixed in v1.74.1 by pointing it at `/`. **The deeper problem is not fixed and is tracked here:** `outputDirectory` publishes only the browser bundle, so `server.mjs` is built on every deploy and discarded — room pages are client-rendered in production and unknown URLs cannot carry a status, which means rows 18 and 20 do not reach the live site. **Removing that setting does not work:** v1.74.0 tried it, Vercel read `outputPath` from `angular.json` and published `dist/mastande-frontend` — whose `index.html` is one directory further down — and the entire site 404'd, `/` included, until it was reverted. The replacement is a `.vercel/output` build (Build Output API), which routes explicitly rather than by framework detection. `scripts/verify-build.sh` notes the static state today and turns it into a failure the moment `frontend/tools/vercel-build.mjs` exists | ⚠️ **Deep links fixed; SSR in production still open** | `frontend/vercel.json`, `docs/DEPLOYMENT.md §4`, `scripts/verify-build.sh` |
