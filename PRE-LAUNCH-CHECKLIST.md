...
| 12 | **The Lighthouse gate has never been green** — it has run twice, both red. Config faults fixed in v1.58.0: two `"//1"`/`"//2"` comment keys sat inside `assertions`, where every key is an audit id, so LHCI failed on "is not a known audit" twice per URL; `render-blocking-resources` was asserted with `maxNumericValue`, which reads NaN on an opportunity audit in Lighthouse 12; and `"preset": "perf"` meant accessibility, best-practices and SEO were never collected, so every assertion naming them had no data. Twelve of the nineteen failures were the config reporting on itself. **The four real findings are fixed in v1.72.0** and measured on the same production build the CI job builds, 4 URLs × 3 runs: accessibility **1.0 on all four** (was 0.96–0.98 — three unnamed `<select>`s, `heading-order` on `/` and `/how-it-works`, `color-contrast` on eight step numbers and the safety note), performance **0.94–0.98** against the 0.9 floor (was 0.76–0.83), LCP **1865–2165ms** against 2500ms (was 3.5–4.2s), and `uses-text-compression` clear now that the SSR server gzips — it never did, so the budget had been measuring a page no visitor receives. **Still not green on a runner, and that is now the only open part:** develop's scheduled run measured LCP 3.47s and performance 0.84 on the same commit that scores 2.1s and 0.96 here, because a GitHub runner is slower than this machine; SEO read 0.92 there and 1.0 here; and `errors-in-console` holds best-practices at 0.96 in both places, from a client-side fetch of `api.umastande.co.za`, a host that does not resolve until the domain is registered. The budgets need re-measuring on a runner against v1.72.0 before either the LCP number or the best-practices floor is judged — do that, not a guess, and do not lower a budget to make it pass. `scripts/a11y-drive.mjs` now covers the accessibility half locally, in a browser, on seven pages rather than Lighthouse's four | ⚠️ **Four findings fixed and verified; the runner's own numbers still to be taken** | `lighthouserc.json`, `frontend/src/server.ts`, `frontend/src/styles/_spec.scss`, `scripts/a11y-drive.mjs` |
...
| 24 | **Does production actually run the SSR server?** `frontend/vercel.json` sets `outputDirectory: dist/mastande-frontend/browser` and rewrites every non-file path to `/index.html`, and `docs/DEPLOYMENT.md §4` documents the same settings — which together describe a **static** deployment of the browser bundle. If that is what the Vercel project is doing, then none of the server runs in production: `/rooms/:id` is client-rendered from the shell rather than server-rendered per request (rows 18 and 20 would have no effect on the live site), the 404 statuses are not applied, and the `allowedHosts` and Cache-Control work in `server.ts` is inert. The prerendered pages (19 of them, including `/`, `/pricing`, `/how-it-works`, the legal pages and the ten locale homes) are served correctly either way, and the robots.txt/sitemap.xml rewrites in row 19 are edge rules that work either way — this is specifically about room pages and statuses. **I cannot see the Vercel project from here, so this is a question, not a finding.** Check whether the deployment has an Angular SSR function; if it does not, the fix is to let the Angular preset deploy the server (drop the explicit `outputDirectory` and the SPA catch-all rewrite, keeping the robots/sitemap ones) or to run `server.mjs` on a host that executes it | ⬜ **Open — needs a look at the Vercel project** | `frontend/vercel.json`, `docs/DEPLOYMENT.md §4` |
